...
/**
 * Transform raw fixture data (including events, stats, players) into DB-ready format
 */
export function transformFixture(apiFixture) {
  ...
  if (apiFixture.teams) {
    for (const side of ["home", "away"]) {
      const team = apiFixture.teams[side];
      if (team) {
        teams.push({
          team_id: team.id,
          name: team.name,
        ...
        fixture_teams.push({
          ...
          team_id: team.id,
          side,
          winner: apiFixture.score?.winner === side,
        });
      }
    }
  }
  ...
  // --- Fixture Scores ---
  const fixture_scores = apiFixture.score ? [{
    fixture_id: fixtureId,
  ...
  const fixture_periods = apiFixture.periods ? [{
    fixture_id: fixtureId,
    first_half_start: apiFixture.periods.first,
    second_half_start: apiFixture.periods.second,
  }] : [];

  // --- Fixture Events ---
  const fixture_events = (apiFixture.events || []).map((event, idx) => ({
    event_id: `${fixtureId}-${idx}`,
    fixture_id: fixtureId,
  ...
  // --- Fixture Stats ---
  const fixture_stats = (apiFixture.statistics || []).flatMap(teamStats =>
  ...
  // --- Fixture Players & Player Stats ---
  const fixture_players = [];
  const fixture_player_stats = [];
